...
describe('/api/admin/ledger-entries*', () => {
  ...
  beforeAll(async () => {
    const society = await prisma.society.create({
      data: { name: `Admin Ledger Society ${suffix}`, address: '1 Test St', upiVpa: 'admin-ledger@okhdfcbank' },
    });
  ...
  describe('GET /api/admin/ledger-entries', () => {
    it('rejects a non-admin token (403)', async () => {
      const res = await request(app).get('/api/admin/ledger-entries').set('Authorization', `Bearer ${ownerToken}`);
