...
describe('/api/admin/dashboard/*', () => {
  ...
  beforeAll(async () => {
    const society = await prisma.society.create({
      data: { name: `Dashboard Route Society ${suffix}`, address: '1 Test St', upiVpa: 'dash-route@okhdfcbank' },
    });
  ...
  describe('GET /api/admin/dashboard/summary', () => {
    it('rejects a non-admin token (403)', async () => {
      const res = await request(app).get('/api/admin/dashboard/summary').set('Authorization', `Bearer ${ownerToken}`);
