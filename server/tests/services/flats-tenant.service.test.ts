...
} from '../../src/services/flats.service';

// Unlike tests/routes/flats-tenant.test.ts, this calls the service directly — no
// HTTP (see tests/services/admin-users.service.test.ts for the same rationale).
describe('flats service — tenant assignment', () => {
  ...
  it('assigns a tenant: opens an OccupancyChange row and sets Flat.currentTenantId', async () => {
    ...
    const occupancy = await prisma.occupancyChange.findFirst({ where: { flatId, effectiveEnd: null } });
  ...
  it('throws InvalidTenantError when tenantId does not exist', async () => {
    await expect(assignTenant(flatId, societyId, 'nonexistent-id')).rejects.toBeInstanceOf(InvalidTenantError);
  });

  it('throws InvalidTenantError when tenantId belongs to a different society', async () => {
    await expect(assignTenant(flatId, societyId, otherSocietyTenantId)).rejects.toBeInstanceOf(InvalidTenantError);
  });

  it('throws InvalidTenantError when tenantId references a non-TENANT user (e.g. the owner)', async () => {
    await expect(assignTenant(flatId, societyId, ownerId)).rejects.toBeInstanceOf(InvalidTenantError);
  });
  ...
  it('throws TenantAlreadyAssignedError when the flat already has a current tenant', async () => {
    ...
    await expect(assignTenant(flatId, societyId, tenantId)).rejects.toBeInstanceOf(TenantAlreadyAssignedError);
  });
  ...
  it('allows re-assigning a (possibly different) tenant after removal', async () => {
    ...
    const openRows = await prisma.occupancyChange.findMany({ where: { flatId, effectiveEnd: null } });
