...
import { approveLedgerEntry, createDeposit, createCredit } from '../../src/services/ledger.service';
...
import { LedgerEntryAlreadyReviewedError, ForbiddenLedgerEntryAccessError } from '../../src/lib/errors';
...
describe('receipt service', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.maintenanceRecord.create({
      data: { flatId, period: '2026-01', payerType: 'OWNER', amount: 1000, dueDate: new Date('2026-01-15'), payerId: ownerId },
    });
  });
  ...
  describe('buildReceiptNumber', () => {
    it('combines the prefix, flat wing+number, and ledger entry id', () => {
      expect(buildReceiptNumber('RCPT', { wing: 'A', flatNumber: '101' }, 'entry123')).toBe('RCPT-A101-entry123');
    });
  });
  ...
  describe('buildReceiptData', () => {
    ...
    it('uses the entry\'s own note as the purpose for a CREDIT', () => {
      const entry = { id: 'e2', type: 'CREDIT' as const, amount: 800, note: 'Repair cost settled', payerId: 'p1' };
      const data = buildReceiptData(entry, flat, payer, society, { date: new Date('2026-08-11') });
  ...
  describe('getSignatureBufferOrUndefined', () => {
    ...
    it('falls back to undefined (does not throw) when the stored key cannot be read', async () => {
      const buffer = await getSignatureBufferOrUndefined({ receiptSignatureFileKey: 'nonexistent/key.png' });
      expect(buffer).toBeUndefined();
    });

    it('reads the actual bytes when the key is valid', async () => {
      const { key } = await getStorageAdapter().save({ buffer: Buffer.from('fake-png-bytes'), societyId, extension: '.png' });
  ...
  describe('previewReceiptPdf', () => {
    ...
    it('renders a preview PDF for a PENDING deposit, with no side effects', async () => {
      ...
      const before = await prisma.receipt.count({ where: { ledgerEntryId: (entry as { id: string }).id } });
      ...
      const after = await prisma.receipt.count({ where: { ledgerEntryId: (entry as { id: string }).id } });
    ...
    it('throws LedgerEntryAlreadyReviewedError for a non-PENDING entry', async () => {
      ...
      await expect(previewReceiptPdf(approved!.id, societyId)).rejects.toThrow(LedgerEntryAlreadyReviewedError);
    });
  ...
  describe('getIssuedReceiptForViewing', () => {
    it('returns null when no Receipt row exists yet (e.g. still PENDING)', async () => {
      ...
      const result = await getIssuedReceiptForViewing((entry as { id: string }).id, ownerId, 'OWNER', societyId);
    ...
    it('throws ForbiddenLedgerEntryAccessError for a different resident', async () => {
      ...
      await expect(getIssuedReceiptForViewing(entryId, 'someone-else-id', 'OWNER', societyId)).rejects.toThrow(
