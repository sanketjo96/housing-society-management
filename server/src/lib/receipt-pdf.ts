...
function drawLogoMark(doc: PDFKit.PDFDocument, x: number, y: number, size: number): void {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    doc.moveTo(px + 2, ly).lineTo(px + iw - 3, ly).stroke();
  }
}
...
export async function renderReceiptPdf(data: ReceiptData, signatureBuffer?: Buffer): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.strokeColor(COLOR.ink).lineWidth(1.2).moveTo(marginX, y).lineTo(marginX + contentWidth, y).stroke();
    y += 18;
    ...
    const towardsHeight = Math.max(14, doc.heightOfString(data.purpose, { width: valueWidth, align: 'right' }));
    ...
    doc.lineWidth(1).roundedRect(marginX, y, boxWidth, boxHeight, 10).fillAndStroke(COLOR.paper, COLOR.line);
    ...
    doc.fillColor(COLOR.ink).font('Helvetica-Bold').fontSize(10);
    doc.text(TRANSACTION_TYPE_LABEL[data.transactionType], valX, by, { width: valueWidth, align: 'right' });
    by += 14 + rowGap;
    ...
    doc.strokeColor(COLOR.line).lineWidth(1).moveTo(labelX, by).lineTo(marginX + boxWidth - boxPad, by).stroke();
    by += rowGap;
