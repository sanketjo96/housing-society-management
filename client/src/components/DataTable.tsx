...
// FlatsListPage, MaintenancePage, and PaymentProofsPage. Deliberately thin: only
// getCoreRowModel (no sorting/filtering/pagination row models) — this is a 24-flat MVP
// (CLAUDE.md: "correctness over scale, don't over-engineer"), every table here has at
// most a handful of rows, so those row models would be unused weight. Callers keep
// full control over cell content (badges, buttons, checkboxes, nested stateful
// components) via each column's `cell` renderer — this component only owns the
// table/thead/tbody chrome and the empty-state row.
